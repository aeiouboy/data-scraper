...
from src.config.category_extractor import extract_category_info
...
async def get_retailer_categories(code: str):
    ...
    try:
        ...
        categories = []
        total_categories = len(retailer_config.category_urls)
        
        for i, url in enumerate(retailer_config.category_urls):
            # Extract category info using the extractor
            category_info = extract_category_info(retailer_config.code, url)
            
            # Calculate estimated products per category
            # Give more weight to major categories
            estimated_products = retailer_config.estimated_products // total_categories
            
            # Adjust estimates for major categories
            major_categories = ['construction', 'tools', 'electrical', 'bathroom', 'kitchen', 
                              'paint', 'tiles', 'lighting', 'plumbing', 'furniture']
            if any(major in category_info['name'].lower() for major in major_categories):
                estimated_products = int(estimated_products * 1.5)
