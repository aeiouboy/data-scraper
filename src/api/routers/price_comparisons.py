    ...
    try:
        ...
        for match in matches:
            if match.get('price_range_min') and match.get('price_range_max'):
                savings_amount = float(match['price_range_max'] - match['price_range_min'])
                ...
                savings_opportunities.append({
                    ...
                    'best_retailer': match.get('best_price_retailer', 'Unknown'),
                    'variance_percentage': match.get('price_variance_percentage', 0)
                })
...
async def get_retailer_competitiveness(request: Request):
    ...
    try:
        ...
        # Analyze competitiveness by category
        competitiveness_by_category = {}
        
        for match in matches:
            category = match.get('unified_category', 'Unknown')
            best_retailer = match.get('best_price_retailer', 'Unknown')
            
            if category not in competitiveness_by_category:
                competitiveness_by_category[category] = {}
            
            if best_retailer not in competitiveness_by_category[category]:
                competitiveness_by_category[category][best_retailer] = {
                    'best_price_count': 0,
                    'total_products': 0,
                    'competitiveness_score': 0.0
                }
            
            competitiveness_by_category[category][best_retailer]['best_price_count'] += 1
            competitiveness_by_category[category][best_retailer]['total_products'] += 1
        
        # Calculate competitiveness scores
        for category_data in competitiveness_by_category.values():
        ...
        return competitiveness_by_category
