...
class ProductMatcher:
    ...
    async def create_product_match(self, master_product: Product, matched_products: List[Product]) -> ProductMatch:
        ...
        # Save to database
        # Convert ProductMatch to dict for Supabase
        match_data = {
            'master_product_id': product_match.master_product_id,
            'matched_product_ids': product_match.matched_product_ids,
            'match_confidence': product_match.match_confidence,
            'match_criteria': product_match.match_criteria,
            'normalized_name': product_match.normalized_name,
            'normalized_brand': product_match.normalized_brand,
            'unified_category': product_match.unified_category,
            'key_specifications': product_match.key_specifications,
            'price_range_min': float(product_match.price_range_min) if product_match.price_range_min else None,
            'price_range_max': float(product_match.price_range_max) if product_match.price_range_max else None,
            'best_price_retailer': product_match.best_price_retailer,
            'price_variance_percentage': product_match.price_variance_percentage
        }
        
        result = self.db.client.table('product_matches').insert(match_data).execute()
    ...
    async def process_all_products(self, retailer_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Get all products using search functionality
        all_products = []
        result = await self.db.search_products(
    ...
    def _normalize_text(self, text: str) -> str:
        """Normalize text for comparison using enhanced normalizer"""
...
class ProductMatchAnalyzer:
    ...
    async def generate_price_comparison_report(self) -> Dict[str, Any]:
        ...
        try:
            result = self.db.client.table('product_matches').select('*').execute()
            matches = result.data if result.data else []
            # Convert dict to ProductMatch objects
            from types import SimpleNamespace
            matches = [SimpleNamespace(**match) for match in matches]
        except Exception as e:
            logger.error(f"Error fetching product matches: {str(e)}")
            matches = []
    ...
    async def get_retailer_competitiveness_analysis(self) -> Dict[str, Any]:
        ...
        try:
            result = self.db.client.table('product_matches').select('*').execute()
            matches = result.data if result.data else []
            # Convert dict to objects with attributes
            from types import SimpleNamespace
            matches = [SimpleNamespace(**match) for match in matches]
        except Exception as e:
            logger.error(f"Error fetching product matches: {str(e)}")
