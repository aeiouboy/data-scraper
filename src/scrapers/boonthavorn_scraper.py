"""
Boonthavorn product scraper implementation
"""
import asyncio
import logging
...
class BoonthavornScraper:
    """Scraper for Boonthavorn products"""
    
    def __init__(self):
        self.firecrawl = FirecrawlClient()
        self.retailer_config = retailer_manager.get_retailer(RetailerType.BOONTHAVORN)
        self.base_url = self.retailer_config.base_url
        
    async def scrape_product(self, url: str) -> Optional[Product]:
        """Scrape a single Boonthavorn product"""
        try:
            ...
            logger.error(f"Error scraping Boonthavorn product {url}: {str(e)}")
            return None
    ...
    def _extract_sku(self, url: str, markdown: str) -> Optional[str]:
        ...
        # Boonthavorn patterns: /product/SKU123 or /p/SKU123 or ?sku=SKU123
        url_patterns = [
            r'/product/([A-Z0-9\-]+)',
            r'/p/([A-Z0-9\-]+)',
            r'/pd/([A-Z0-9\-]+)',
            r'[?&]sku=([A-Z0-9\-]+)',
            r'/([A-Z0-9]{6,})',  # 6+ character alphanumeric code in URL
        ]
    ...
    def _extract_product_name(self, markdown: str, metadata: Dict) -> Optional[str]:
        ...
        if metadata.get('title'):
            ...
            title = re.sub(r'\s*-\s*Boonthavorn.*$', '', title, re.IGNORECASE)
            title = re.sub(r'\s*ราคา.*$', '', title)  # Remove price info
            title = re.sub(r'\s*\d+\s*บาท.*$', '', title)  # Remove price
            title = re.sub(r'\s*บุญถาวร.*$', '', title)  # Remove Thai company name
            return title.strip()
    ...
    def _extract_prices(self, markdown: str, html: str) -> Dict[str, Optional[Decimal]]:
        ...
        # Price patterns for Boonthavorn
        price_patterns = [
            r'฿\s*([0-9,]+(?:\.\d{2})?)',
            r'THB\s*([0-9,]+(?:\.\d{2})?)',
            r'ราคา[:\s]+([0-9,]+(?:\.\d{2})?)',
    ...
    def _extract_brand(self, markdown: str, product_name: str) -> Optional[str]:
        ...
        # Boonthavorn often has brand at the beginning
        words = product_name.split()
        if words and words[0].isupper() and len(words[0]) > 2:
            # Check if it's likely a brand (not a common word)
            common_words = ['THE', 'NEW', 'SET', 'ชุด', 'เซต', 'อุปกรณ์', 'กระเบื้อง', 'สุขภัณฑ์']
            if words[0] not in common_words:
                # Common brands at Boonthavorn
                known_brands = ['COTTO', 'KOHLER', 'AMERICAN', 'TOTO', 'GROHE', 'HAFELE', 'DURAVIT', 'IDEAL']
                if words[0] in known_brands or len(words[0]) >= 4:
                    return words[0]
    ...
    def _extract_features(self, markdown: str) -> List[str]:
        ...
        feature_sections = re.findall(
            r'(?:คุณสมบัติ|Features?|จุดเด่น|ข้อดี|ประโยชน์|คุณลักษณะ).*?\n((?:[-•*✓]?\s*[^-•*✓\n][^\n]+\n?)+)',
            markdown,
    ...
    def _extract_specifications(self, markdown: str) -> Dict[str, Any]:
        ...
        # Common spec keywords for Boonthavorn (ceramic & sanitary specialist)
        spec_keywords = [
            'ขนาด', 'น้ำหนัก', 'สี', 'วัสดุ', 'รุ่น', 'ผิว', 'เกรด',
            'Size', 'Weight', 'Color', 'Material', 'Model', 'Surface', 'Grade',
            'Dimensions', 'Width', 'Height', 'Length', 'Depth', 'Thickness',
            'ความหนา', 'กว้าง', 'ยาว', 'สูง', 'ลึก', 'เส้นผ่านศูนย์กลาง',
            'การใช้งาน', 'Usage', 'Application', 'พื้นที่ใช้งาน',
            'ชนิด', 'Type', 'Style', 'รูปแบบ', 'ลาย', 'Pattern',
            'มาตรฐาน', 'Standard', 'การรับประกัน', 'Warranty',
        ]
    ...
    def _extract_availability(self, markdown: str) -> str:
        ...
        # Boonthavorn availability patterns
        in_stock_patterns = [
            r'มีสินค้า',
            r'พร้อมส่ง',
            r'In Stock',
        ...
        # Check for stock in showrooms
        showroom_match = re.search(r'โชว์รูม.*มี\s*\d+', markdown)
        if showroom_match:
            return 'in_stock'
    ...
    def _extract_images(self, firecrawl_result: Dict[str, Any]) -> List[str]:
        ...
        if 'images' in firecrawl_result:
            for img in firecrawl_result['images']:
                ...
                skip_patterns = ['logo', 'banner', 'icon', 'loading', 'placeholder', 'payment', 'delivery', 'brand-logo', 'social']
                if not any(pattern in img_url.lower() for pattern in skip_patterns):
                    # Boonthavorn specific: look for product images
                    if any(pattern in img_url for pattern in ['/product/', '/p/', '/item/', 'product-', '/media/', '/catalog/']):
                        images.append(img_url)
        
        return images[:10]  # Limit to 10 images
    
    def _map_to_unified_category(self, category: Optional[str]) -> str:
        """Map Boonthavorn category to unified category"""
        if not category:
            return 'other'
        
        category_lower = category.lower()
        
        # Mapping based on Boonthavorn's focus on ceramic & sanitary
        mappings = {
            'bathroom': ['ห้องน้ำ', 'bathroom', 'สุขภัณฑ์', 'sanitary', 'อ่างล้างหน้า', 'อ่างอาบน้ำ', 'ฝักบัว', 'shower'],
            'kitchen': ['ครัว', 'kitchen', 'เครื่องครัว', 'อ่างล้างจาน', 'sink', 'ก๊อกน้ำ'],
            'flooring': ['พื้น', 'floor', 'กระเบื้อง', 'tile', 'ceramic', 'เซรามิก', 'แกรนิต', 'granite', 'หินอ่อน', 'marble'],
            'lighting': ['โคมไฟ', 'lighting', 'หลอดไฟ', 'ไฟ', 'light'],
            'doors_windows': ['ประตู', 'หน้าต่าง', 'door', 'window', 'บานพับ', 'กลอน'],
            'plumbing': ['ประปา', 'plumbing', 'ท่อ', 'pipe', 'วาล์ว', 'valve'],
            'decor': ['ตกแต่ง', 'decor', 'decoration', 'ของแต่งบ้าน'],
            'outdoor': ['สวน', 'outdoor', 'กลางแจ้ง', 'garden'],
            'paint': ['สี', 'paint', 'ทาสี'],
            'construction': ['ก่อสร้าง', 'construction', 'building', 'ปูน', 'cement'],
        }
        ...
        return 'flooring'  # Default for Boonthavorn (ceramic specialist)
    ...
    def _determine_monitoring_tier(self, price: Optional[Decimal]) -> str:
        ...
        # Boonthavorn focuses on ceramic tiles and sanitary ware
        # Mid-range pricing for quality products
        if price_float > 20000:
    ...
    async def scrape_batch(self, urls: List[str]) -> Dict[str, Any]:
        ...
        try:
            for i, url in enumerate(urls):
                if i > 0 and i % 10 == 0:
                    logger.info(f"Batch progress: {i}/{len(urls)} products scraped")
    ...
    async def scrape_category(self, category_url: str, max_pages: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully scraped {success_count} products from {category_url}")
    ...
    async def _discover_product_urls(self, category_url: str, max_pages: int) -> List[str]:
        ...
        for page in range(1, max_pages + 1):
            try:
                # Build page URL (Boonthavorn uses different pagination patterns)
                if '?' in category_url:
                ...
                for link in links:
                    ...
                    # Boonthavorn product URL patterns
                    if any(pattern in url for pattern in ['/product/', '/p/', '/pd/', '/item/', '/catalog/']):
